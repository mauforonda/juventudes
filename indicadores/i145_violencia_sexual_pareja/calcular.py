"""Calcula la violencia sexual de pareja en los últimos 12 meses."""
...
ACTOS = [f"acto_{letra}" for letra in "GHI"]
COLUMNAS_RESULTADO = [
    "gestion",
    *DIMENSIONES_JOVENES,
    "observaciones",
...
def calcular():
    datos = cargar_individuales(
        {f"acto_{letra}": f"ms11_1108_{letra}" for letra in "GHI"},
        {f"acto_{letra}": f"vs08_0808_{letra}" for letra in "GHI"},
    )
