"""Calcula la presencia de hambre en hogares con jóvenes."""
...
def calcular():
    datos = cargar_personas_con_hogar(
        "EH2025_Seguridad_Alimentaria.parquet", columnas_hogar=HAMBRE
    )
    return estimar_porcentaje(
        datos,
        caso=lambda d: d[HAMBRE].eq(1).any(axis=1),
        valido=lambda d: d[HAMBRE].isin([1, 2]).all(axis=1),
